...
const progressController = {
    ...
    startSection: asyncHandler(async (req, res) => {
        ...
        if (!courseProgress) {
            return res.status(404).json({ message: "Course not found in user's progress" });
        }
        ...
        await user.save();
        // ! send the res
        res.status(200).json({ message: "Section started sucessfully" })
    }),
    ...
    update: asyncHandler(async (req, res) => {
        ...
        const userId = req.user;
        const user = await User.findOne({ _id: userId, "progress.courseId": courseId, });
        if (!user) {
            return res.status(404).json({ message: "User or course not found" });
        }
        // ! Find and update specific secton status
        const courseProgress = user.progress.find((p) => p.courseId.toString() === courseId);
        const sectionProgress = courseProgress.sections.find((s) => s.sectionId.toString() === sectionId);
        if (sectionProgress) {
            sectionProgress.status = newStatus;
            await sectionProgress.save();
        } else {
            return res.status(404).json({ message: "Section not found in user's progress" });
        }
        await user.save();
        // ! Send the response
